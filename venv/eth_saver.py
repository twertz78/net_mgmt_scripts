...
class EthSaver(object):
            ...
            if len(files) >= 2:
                ...
                try:
                    if self.protocol == "ftp":
                        s = cli(
                            "copy bootflash:%s ftp://%s:%s@%s/%s vrf %s" %
                            (files[0], self.username, self.password, self.server, files[0], self.vrf)
                        )
                    else:
                        s = cli(
                            "copy bootflash:%s tftp://%s/%s vrf %s" %
                            (files[0], self.server, files[0], self.vrf)
                        )
    ...
    def execute_capture(self):
        ...
        while file_count < self.max_files:
            ...
            cli(
                "ethanalyzer local interface inband limit-captured-frames %d write bootflash:%s" %
                (self.max_frames, file_name)
            )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="mode_capture", help="run this program in capture mode",
        default=False
    )
    parser.add_option(
        "-e", "--export", action="store_true",
        dest="mode_export", help="run this program in export mode",
        default=None
    )
    parser.add_option(
        "--server", action="store", type="string",
        dest="server", help="server name for export mode",
        default=None
    )
    parser.add_option(
        "--protocol", action="store", type="string",
        dest="protocol", help="protocol for export mode (can be ftp or tftp)",
        default="ftp"
    )
    parser.add_option(
        "--username", action="store", type="string",
        dest="username", help="username for ftp export",
        default=None
    )
    parser.add_option(
        "--password", action="store", type="string",
        dest="password", help="password for ftp export",
        default=None
    )
    parser.add_option(
        "--frames", action="store", type="int",
        dest="max_frames", help="maximum number of frames per capture",
        default=EthSaver.MAX_FRAMES
    )
    parser.add_option(
        "--files", action="store", type="int",
        dest="max_files", help="maximum number of files to capture/export",
        default=EthSaver.MAX_FILES
    )
