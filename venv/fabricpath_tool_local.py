...
if topology == 31:
    ...
    sys.exit("Your selection was not valid, please use 31, 63, 11, 12, 22, 1, 7, 35, 20, 29, or 6")
...
for switch in switches:
    ...
    vlans_in_topo_6 = device.send_command_expect("show fabricpath isis topology %s vlan-range" % topology)
...
new_file.write("logfile = file('fabricpath_" + topology + ".txt', 'w')" + '\n')
new_file.write('i = 0' + '\n')
...
new_file.write('        logfile.write("no switches contain VLAN %i in topology ' + topology + '" % i)' + '\n')
...
new_file.write('        logfile.write("ALL SWITCHES CONTAIN VLAN %i IN TOPOLOGY ' + topology + '" % i)' + '\n')

new_file.write('    else:' + '\n')
new_file.write('        logfile.write("VLAN %i only exists in topology ' + topology + ' on switches: " % i)' + '\n')
m = 0
while m < i:
